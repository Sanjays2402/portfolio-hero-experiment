...
export default function Home() {
  return (
    <div className="min-h-screen bg-black text-white px-6 lg:px-16 py-16 lg:py-24">
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-6xl">
        {variants.map((v, i) => (
          <motion.div key={v.slug}
            initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
            <Link href={`/${v.slug}`} className="group relative block p-8 rounded-2xl border border-white/10 hover:border-white/30 hover:bg-white/[0.03] transition overflow-hidden">
              ...
              <h2 className="mt-6 text-3xl font-semibold tracking-tight">{v.name}</h2>
