...
interface Props {
  /** The animated centerpiece (canvas component). */
  centerpiece: ReactNode
  /** Label shown in the bottom-left badge, e.g. "Variant 02 — Wireframe Globe". */
  variantLabel?: string
}

export function HeroShell({ centerpiece, variantLabel }: Props) {
  return (
    <section
      id="home"
      className="relative min-h-[92vh] flex items-center justify-center pt-28 pb-32 overflow-hidden"
    >
      <div className="absolute inset-0 pointer-events-none">{centerpiece}</div>

      {/* dimming + radial center scrim */}
      <div className="absolute inset-0 bg-gradient-to-b from-background/40 via-background/10 to-background pointer-events-none" />
      <div
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[440px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at center, hsl(var(--background) / 0.78) 0%, transparent 70%)' }}
      />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        >
          ...
          >
            ...
            <span
              className="block italic font-light text-foreground/60"
      ...
      {/* variant label badge */}
      {variantLabel && (
        <div className="absolute top-24 left-6 z-30 hidden lg:block">
          <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-background/70 backdrop-blur-xl border border-border/60 text-xs font-mono text-muted-foreground">
      ...
      >
        <div className="flex animate-marquee whitespace-nowrap">
          {[...taglineWords, ...taglineWords].map((w, i) => (
            <span key={i} className="mx-6 text-xs tracking-[0.3em] uppercase text-muted-foreground">
              {w}<span className="ml-12">&bull;</span>
            </span>
          ))}
        </div>
      </motion.div>
